...
def process_xfa_estars(doc_id, bucket, path):
    """
    :param object_file_metadata:
    :param deserialized_object:
    :param premarket_bucket:
    ...
    obj = s3_client.get_object(Bucket=bucket, Key=f"{path}/{doc_id}.pdf")
    print(f"getting object {bucket}/{path}/{doc_id}.pdf")
    pdf = io.BytesIO(obj["Body"].read())
